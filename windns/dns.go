...
func (c *Client) Insert(host, zone string, reqs []string) (r *dnsc.Msg, tt time.Duration, err error) {
	msg := new(dnsc.Msg)
	updates := []dnsc.RR{}

	for _, req := range reqs {
		var rr dnsc.RR
		if rr, err = dnsc.NewRR(req); err != nil {
			return
		}
		updates = append(updates, rr)
	}

	msg.SetUpdate(dnsc.Fqdn(zone))
	msg.Insert(updates)
...
func (c *Client) Remove(host, zone string, reqs []string) (r *dnsc.Msg, tt time.Duration, err error) {
	msg := new(dnsc.Msg)
	updates := []dnsc.RR{}
...
func (c *Client) Update(host, zone string, oReqs, nReqs []string) (r *dnsc.Msg, tt time.Duration, err error) {
	msg := new(dnsc.Msg)
	oUpdates := []dnsc.RR{}
	nUpdates := []dnsc.RR{}
...
func (c *Client) Lookup(host, value string) (data string, r *dnsc.Msg, tt time.Duration, err error) {
	...
	data = strings.Trim(dnsc.Field(r.Answer[0], 1), ".")
	return
}

// Exchange performs an exchange
func (c *Client) Exchange(host string, msg *dnsc.Msg) (r *dnsc.Msg, tt time.Duration, err error) {
	msg.SetTsig(*c.Keyname(), tsig.GSS, 300, time.Now().Unix())

	r, tt, err = c.client.Exchange(msg, net.JoinHostPort(host, "53"))
	if r.Rcode != dnsc.RcodeSuccess {
		err = fmt.Errorf("DNS error: %s (%d)", dnsc.RcodeToString[r.Rcode], r.Rcode)
		return
	}
